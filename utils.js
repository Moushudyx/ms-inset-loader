...
/**
 * 在`template`中用正则匹配并替换一段代码
 * @param {string} template 模板代码
 * @param {string} labelCode 要添加的代码
 * @param {RegExp} regLabel 根据正则确认添加位置
 * @returns {string}
 */
const generateHtmlCode = (template, labelCode, regLabel) => {
  // 去除`template`中所有注释和首尾空白
 ...
 * @param {require('vue-template-compiler').SFCDescriptor.styles} styles
 */
